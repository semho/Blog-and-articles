...
<div class="wrapper-table-users">
    <table class="table table-striped table-hover table-users">
        ...
        <tbody>
            <?php foreach ($users as $keyId => $curentUser): ?>    
                <tr>
                    <th scope="row"><?=$keyId?></th>
                    <td><?=$curentUser['full_name']?></td>
                    <td><?=$curentUser['email']?></td>
                    <td><?=$curentUser['group']?></td>
                    <td><?=$curentUser['subscription']?></td>
                    <td>
                        <a href="/admin/users/<?=$keyId?>">
                            <img src="/img/users/change.png" width="20px" alt="Редактировать">
                        </a>
                    </td>
                    <td>
                        <a href="#" class="link-delete-user" data-bs-user="<?=$keyId?>">
        ...
        <nav class="nav-pagination mt-4" aria-label="Page navigation">
            <ul class="pagination">
                <li class="page-item <?=($start == 0) ? 'disabled' : ''; ?>">
                    <a class="page-link" aria-label="Previous" href="/admin/users?page=<?=($page - 1)?>&count=<?=$countRecords?>" >
                        <span aria-hidden="true">&laquo;</span>
                    </a>
                </li>
                <?php for ($i = 1; $i <= $pages; $i++) : ?>
                    <li class="page-item <?=($page == $i) ? 'active' : ''; ?>">
                        <a class="page-link" href="/admin/users?page=<?=$i?>&count=<?=$countRecords?>"><?=$i?></a>
                    </li>
                <?php endfor ?>
                <li class="page-item <?=(($count - $start) <= $countRecords) ? 'disabled' : ''; ?>">
                    <a class="page-link" aria-label="Next" href="/admin/users?page=<?=($page + 1)?>&count=<?=$countRecords?>" >
