...
<h1><?=$title?></h1>
<form action="/admin/pages/<?=$changeablePage->id?>" method="post" id="formChangePage" enctype="multipart/form-data">
    <div class="row">
        <div class="col-xl-12">
            <div>
                <label class="small mb-1" for="adminPageAddTitle">Заголовок</label>
                <input class="form-control mb-3 <?php if (isset($errors['name']) || isset($errors['slug'])): ?>error--input<?php endif ?>" id="adminPageAddTitle" type="text" name="name" placeholder="Введите Название" value="<?=$changeablePage->title?>">
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control mb-3 <?php if (isset($errors['text'])): ?>error--input<?php endif ?>" name="text" id="textPageAddAdmin"><?=$changeablePage->text?></textarea>
